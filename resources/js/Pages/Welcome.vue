...
<template>
    ...
    >
        <div v-if="canLogin" class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
            ...
            <template v-else>
                <Link :href="route('login')" class="text-sm text-green-700 dark:text-green-500 underline">Log in</Link>

                <Link
                    v-if="canRegister"
                    :href="route('register')"
                    class="ml-4 text-sm text-red-700 dark:text-red-500 underline"
                    >Register</Link
                >
            </template>
        </div>
